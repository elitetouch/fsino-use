  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <p className="pt-1 text-[0.625rem] font-bold uppercase tracking-[0.14em] text-[var(--color-brand-muted-soft)]">
          {label}
        </p>
      </div>
      <div className="mt-3 flex items-end justify-between gap-2">
        <p className="text-[1.25rem] font-bold leading-none tracking-tight text-[var(--color-brand-fg)] sm:text-[1.375rem]">
          {value}
        </p>
        {trend && (
          <span className={cn('shrink-0 pb-0.5 text-[0.625rem] font-semibold uppercase tracking-wider', TREND_TONE[trend.tone ?? 'neutral'])}>
            {trend.value}
          </span>
        )}
      </div>
      {sub && (
        <p className="mt-1 text-[0.75rem] text-[var(--color-brand-muted)]">{sub}</p>
      )}
    </div>
  );
}
