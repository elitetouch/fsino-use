...
>(function Input({ className, ...props }, ref) {
  return (
    <input
      ref={ref}
      className={cn(
        'block h-10 w-full rounded-[10px] border border-[var(--color-brand-input-border)] bg-white px-3 text-[0.875rem] text-[var(--color-brand-fg)] placeholder:text-[var(--color-brand-muted)]/70',
...
>(function Label({ className, ...props }, ref) {
  return (
    <LabelPrimitive.Root
      ref={ref}
      className={cn(
        'mb-1 block text-[0.75rem] font-semibold text-[var(--color-brand-fg-soft)]',
        className,
      )}
      {...props}
    />
...
export function FieldError({ message }: { message?: string }) {
  ...
  return (
    <p className="mt-1 text-[0.6875rem] font-medium text-[var(--color-brand-danger)]">
      {message}
    </p>
  );
}
